...
~~2-4段压缩 + 底部 `<details>` 折叠技术依据~~ — case-01 首次渲染翻车：5 个 finding 压成 4 段，丢了辅星/小星/碰撞/因果組细节。反馈"不够详细，还不如 finding 详细""没做引用"。重写对齐希腊化格式后 用户确认"这个输出才是读了就知道在说的是 subject"。
...
技术引用放在每小节末尾 `技术依据:` 行（inline，不折叠）。**正文零术语，技术依据行可以用术语。**
