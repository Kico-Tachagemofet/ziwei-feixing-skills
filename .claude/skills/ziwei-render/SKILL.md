...
> v1.5 (2026-05-24) — Render 输出量化基准 + Step 7 强制自查 + #11 触发条件强制化 (Z01 new session 翻车修复)
...
## ⚠️ 详细度基准 (对标 Z08-kico-mom — v1.5 新增, READ FIRST)

**Render 失败模式**: 把每个 topic 压成 1-2 节概述, 文件 < 5 KB, finding 信息 70-80% 丢失。

**对标 Z08-kico-mom 实际产出 (合格基准)**:

| Topic | 文件大小 | 小节数 | 每小节段数 |
|---|---|---|---|
| character (5 finding) | 14 KB | 5 节 | 4-6 段 |
| partnership (6 finding) | 17 KB | 6 节 | 4-6 段 |
| career (5 finding) | 14 KB | 5 节 | 4-6 段 |
| 短 topic (3 finding) | 9 KB | 3 节 | 3-4 段 |

**反例 (绝对禁止)**:
- Z01-astarion new session: partnership 6 finding → 1 节 2.7 KB (Z08 同等是 15.5 KB / 5+ 节). **差 5-6 倍, 严重违规**.
- 根本原因: 没读详细度基准 + 没跑 Step 7 自查

**核心铁律**:
1. **小节数 == finding 数 (1:1)** — 6 finding 必产 6 节, 不允许压缩成 1-2 节
2. **每小节 4-6 段** — 不是 2-3 段, 不是概述
3. **每 topic 文件 ≥ 6 KB (短) / ≥ 10 KB (复杂)** — 不达标 = 几乎肯定压缩了
4. **Step 7 强制自查** — 写完每个 topic 必跑, 不达标立即重写
...
[**4-6 段**叙事散文 — 不是 2-3 段, 不是概述, 是把 finding 里每个具体画面/小星/碰撞/因果都展开]
...
### Step 7: Render 输出自查 (v1.5 新增, MANDATORY 强制)

⚠️ **写完每个 topic reader 文件后必跑此自查。不达标 → 立即重写该 topic, 不允许 ship。**

#### 量化基准 (对标 Z08-kico-mom)

| 检查项 | 短 topic (≤ 3 finding) | 复杂 topic (≥ 4 finding) | 失败处置 |
|---|---|---|---|
| **小节数** | = finding 数 (1:1) | = finding 数 (1:1) | **不达标 = 立即重写** |
| **每小节段数** | ≥ 3 段 ≥ 150 字 | ≥ 4 段 ≥ 200 字 | 缺则重写该小节 |
| **文件大小** | ≥ 6 KB | ≥ 10 KB | 不达标 = 检查是否压缩 → 重写 |
| **bold hook** | 必有 1 句 | 必有 1 句 | 缺则补 |
| **<!-- 生活判断 --> 注释** | 每小节必有 | 每小节必有 | 缺则补 |
| **inline 技术依据** | 每小节必有 | 每小节必有 | 缺则补 |
| **N 线共振 / RC 对照表 / cross-ref** | 至少 1 处 (when 高对比度 finding) | 至少 1-2 处 | 缺则审视是否漏 #11 触发 |
...
```
□ 小节数 == finding 数 (找 topic-findings/<slug>.md 数 finding 数, 对应 reader 小节数)
□ 每小节段数 ≥ 4 段 (复杂 topic) / ≥ 3 段 (短 topic)
□ 每小节有 bold hook (## 标题下第一句)
□ 每小节有 <!-- 生活判断 --> 注释
□ 每小节末尾有 inline `技术依据:` 行
□ topic 文件 ≥ 6 KB (短) / ≥ 10 KB (复杂)
□ ≥3 信号汇聚的 finding 用了 #11 N 线共振或 RC 对照表
□ cross-reference 数量 ≥ 2 处 (引到其他 topic 或 composition §X)
□ Privacy: friend-proxy 模式时 命名/署名/来源 全中性化
□ Subject-context informed 段落 strip 来源标注
```

**任何一项失败 → 标记本 topic 为"需重写", 不进 Step 4 cross-topic 收尾**。

#### Render 输出 audit (可选自动化)

调 `ziwei-finding-audit` skill 的 render audit 模式 (v2.2+):
- 输入: `reader/<XX>-<topic>.md` + 对应 `topic-findings/<slug>.md`
- Audit 维度: 上述量化基准 + #11 触发完整性 + cross-ref 网络

#### Anti-pattern (绝对禁止)

❌ "5 个 finding 压成 1 节概述" — 已发现的最严重违规, finding 信息 70-80% 丢失
❌ "每小节 1-2 段" — 远低于 ≥ 4 段标准
❌ "topic 文件 < 5 KB" — 几乎肯定是压缩了
❌ "高对比度 finding 用散文糊" — #11 适用场景不用 #11 = 浪费高对比度信号

**违规者范例**: Z01-astarion new session render 把 6 finding partnership 压成 1 节 2.7 KB (Z08 同等盘是 15.5 KB / 5+ 节 / 4-6 段)。差 5-6 倍。原因: 没跑 Step 7 自查。

→ **必须跑 Step 7。 写完不自查 = 半成品**。
...
## First-invocation Flow (v1.5)

0. **READ FIRST**: 读本 SKILL.md 顶部 "详细度基准 (对标 Z08-kico-mom)" 段, 记住量化基准 (4-6 段 / 6-10+ KB / 小节数=finding 数)
...
3. 确认 topic-findings/ 下有哪些 topic, **数清每 topic 的 finding 数** (后续 self-check 用)
4. 按 topic 顺序逐个渲染: Step 1 → 1.5 → 2 → 3 (引用 composition.md 各段)
5. **每写完 1 个 topic → 立即跑 Step 7 自查** (mandatory, 不达标重写)
6. Step 4 cross-topic 收尾 (cross-reference 网络 + 去重 + 统一收束)
7. Step 5 subject-context handling (Stage 2/3 已 mark 的段保留共振结构 strip 来源)
8. Step 6 privacy 规则 (friend-proxy 模式触发 → 中性命名 + 去署名 + 不提来源)
9. **全部 topic 完成后再跑一遍 Step 7 全局自查** (检查 cross-topic 完整性)
10. 报告: N 个 topic 渲染完成 + 每 topic 自查结果表 + reader/ 文件列表 + report.html
