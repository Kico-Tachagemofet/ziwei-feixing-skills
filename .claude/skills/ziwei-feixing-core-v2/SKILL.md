...
Source-cards 路径: `source-cards/` (repo-relative; publish 仓库已含完整 source-cards 树)
...
Main Taiji 必须等于 lens Main Taiji。次太极最高 Secondary，全局最高 Background。
...
Lens Packet 必须有宫对索引。缺 → 回退 topic-lens。Core 不补做信号收集。
