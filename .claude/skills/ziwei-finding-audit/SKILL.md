...
> v2.2 (2026-05-24) — 扩展支持 Stage 4 render audit (使用 render SKILL Step 7 量化基准 + 详细度对标 Z08)
...
> - **Stage 4 render audit (v2.2 新增)**: 每个 reader/<XX>.md 产出后 (用 ziwei-render SKILL Step 7 量化基准 checklist)
...
### Stage 4 render audit 模式 (v2.2 新增)
| 文件 | 必需 |
|---|---|
| reader/<XX>-<topic>.md | 是 — 审计对象 (每个 topic 单独 audit) |
| 对应 topic-findings/<slug>.md | 是 — finding 数 vs 小节数 1:1 验证 |
| composition.md | 是 — cross-reference 验证 |
| Render Quality Schema | 参照 `ziwei-render/SKILL.md` Step 7 量化基准 + render-checklist.md #9/#11 |

### Render audit 检查项 (R 层)

| ID | 检查 | 失败级别 |
|---|---|---|
| **R1 小节数 1:1** | reader 小节数 == topic-findings finding 数 ? | **BLOCKER** |
| **R2 每小节段数** | 复杂 topic ≥ 4 段 / 短 topic ≥ 3 段 ? | **BLOCKER** |
| **R3 文件大小** | 复杂 (≥4 finding) ≥ 10 KB / 短 ≥ 6 KB ? | **BLOCKER** |
| R4 bold hook | 每小节标题下第一句有 bold hook ? | WARNING |
| R5 生活判断注释 | 每小节有 `<!-- 生活判断: -->` HTML 注释 ? | WARNING |
| R6 inline 技术依据 | 每小节末尾有 `技术依据:` 行 ? | WARNING |
| **R7 #11 触发完整性** | ≥3 信号汇聚 finding 用了 N 线共振 / RC 对照表 ? | **BLOCKER** |
| R8 cross-reference 网络 | 每 topic 至少 ≥ 1 处 cross-ref ? | WARNING |
| R9 隐私规则 (friend-proxy) | 命名/署名/来源标注是否中性化 ? | WARNING |
| R10 subject-context strip | ✨ informed 段保留共振结构, strip 来源标注 ? | WARNING |
| R11 术语外露 | 正文出现"管束/缘起/收束/归藏/化禄/化忌"等机制词 ? | WARNING |
| R12 改 finding 方向 | render 是否改变了 finding 的判断方向 ? | **BLOCKER** |
...
⚠️ **核心反例 (Z01-astarion new session)**:
- partnership: 6 finding 压成 1 节 2.7 KB → R1 (小节数 1≠6) + R2 (1 节 < 4 段) + R3 (2.7 KB < 10 KB) 三 BLOCKER 同时 FAIL
- 后果: render 输出 70-80% finding 信息丢失
- 修复: 拆成 6 节 × 4-6 段 → ~15 KB (与 Z08 同 topic 持平)
