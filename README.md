...
本 skill 系统引用 **source-cards**（许铨仁飞星体系的结构化笔记），分为短卡 (88 张 bundle) 和深卡 (deep)。

这些内容需要你自己从许铨仁的著作中整理（或用其他方式获取），结构如下:
...
安装后需在以下文件中替换 `<YOUR_SOURCE_CARDS_PATH>` 为实际路径:
- `ziwei-feixing-core-v2/SKILL.md`
- `ziwei-source-lookup/SKILL.md`

王亭之 addon 需在 `ziwei-wangtingzhi-minor-stars-addon/SKILL.md` 中替换 `<YOUR_EPUB_PATH>` 为你的 EPUB 路径。
...
许铨仁飞星派的理论基础来自许铨仁先生的著作，source-cards 内容需自行获取。
