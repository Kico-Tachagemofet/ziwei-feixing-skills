...
## Codex 可选项目文件：尝试解决“原生机房创伤”

本独立分支提供两份**经过脱敏的实验性项目文件**，供使用 Codex 跑这套 pipeline 的用户选用。这里用“原生机房创伤”形容实际使用中遇到的一类执行与表达偏向：

- 查资料、列证据和跑审计投入很多，最后却没有形成清楚的主判断。
- 为了面面俱到，把总问题拆碎；用同义复述满足篇幅，读者获得的信息没有增加。
- 为避免出错，把有依据的具体判断磨成“有联系、要整理、存在不同层面”等泛泛表述。
- 用户要求讲清配置后，又把正文改成星义字典、四化明细或审计底稿，让读者自己拼出结论。
- 核对了格式、引用与上游一致性，却没有同时检查是否真正回答了占问。

这两份文件以正向引导的方式，把注意力放回**合参、主次、具体人事判断与读者理解**。它们不修改 skill、术理规则、pipeline、来源门禁、篇幅格式或审计要求，也不预设任何命盘的结论。它们是执行层面的辅助尝试，效果仍需在实际任务中检验。

| 文件 | 用途 |
|---|---|
| [AGENTS.md](docs/codex/AGENTS.md) | 简短的项目协作入口：先回答占问，用证据支撑判断，明确主代理的整合责任。 |
| [判读协作指南.md](docs/codex/判读协作指南.md) | 按需参照的具体做法：保持总问题、完成合参、组织正文、从证据和读者两端审查，并定位反馈所在阶段。 |

### Codex 用户如何使用

适用于**已经在 Codex 中配置好这套紫微 skills 和 source-cards**的分析项目。这两份文件只提供项目协作引导，不是自动安装或运行环境适配脚本。

1. 将上表中的两份文件放到你实际开展紫微分析的**项目根目录**，保留文件名和同级关系：
...
2. 如果项目已有 `AGENTS.md`，先保留原文件，将模板中适用的协作约定合并进去，保留原有项目规则。`判读协作指南.md` 放在同一目录，确保入口中的相对链接有效。
3. 从该项目启动新的 Codex 任务，让它概述适用的项目协作要求。应能看到它说明“遵循既有 skills 和 pipeline、形成有依据的具体判断、就近解释必要配置、按需读取指南”。已有任务也可明确让它读取这两份文件。

`AGENTS.md` 的加载及与现有指令的关系，见 [OpenAI 官方项目指令说明](https://learn.chatgpt.com/docs/agent-configuration/agents-md)。文件放在分析项目中即可，无需复制到全局指令目录。公开模板不包含使用者姓名、本机绝对路径、私人案例或历史复盘链接；可按自己的项目补充工作范围与资料边界。
...
### 2026-05-25: fix — core 太极点选择泄漏修复

**问题**: `ziwei-feixing-core-v2` 在缺少 Lens Packet 时会绕过 `ziwei-topic-lens`，自行选择太极点并继续流程，导致太极点精度下降、宫对索引缺失。

**修复**:
- `feixing-minimal-grammar.md` 第 3 步: 原"选太极点"改为明确标注 lens skill 职责，core 不自行选
- `SKILL.md` Gate 1: 升级为硬门禁 — 无 Lens Packet = STOP，必须 invoke lens
- `SKILL.md` 生产硬约束: 新增第 8 条"太极点选择不可自行代劳"
- `SKILL.md` Step 0.5: 强制校验 — 缺 Packet 或无宫对索引 → STOP invoke lens，不自行补做

**涉及 skill**: ziwei-feixing-core-v2 (core + grammar + pipeline-spec), ziwei-topic-lens, ziwei-finding-audit, ziwei-render
